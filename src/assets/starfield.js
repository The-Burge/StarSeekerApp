    ...
    return (
      <Animated.View
        style={{
          ...
          transform: [{scale: this.props.anim.scale}],
        }}>
        {stars}
      </Animated.View>
    );
...
export default class Starfield extends PureComponent {
  constructor(props) {
    super(props);
    const anims = Array(NUM_LAYERS)
      .fill()
      .map((_, idx) => {
        const phase = idx / NUM_LAYERS;
        const zPos = new Animated.Value(1.0);
        const zp = Animated.add(
          Animated.modulo(Animated.add(zPos, phase), 1),
          0.1,
        );
